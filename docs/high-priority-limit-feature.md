...
**Важно:** Эта функциональность работает в дополнение к существующему лимиту в 10 важных задач для Google Sheets интеграции.
...
### 4. Добавление функциональности редактирования задач

#### Кнопка "Редактировать"
Добавлена во все места выбора приоритета:
- Автоматические уведомления о новых задачах
- Команда `/prioritize`

#### Обработчик редактирования
- `handle_edit_task_callback` - обрабатывает нажатие кнопки "Редактировать"
- Активирует режим редактирования для пользователя
- Сохраняет `pending_id` в контексте пользователя

#### Логика редактирования в `handle_message`
- Проверяет режим редактирования пользователя
- Принимает следующее сообщение как новый текст задачи
- Обновляет задачу в базе данных
- Отправляет новое сообщение с выбором приоритета
- Удаляет исходное сообщение редактирования
